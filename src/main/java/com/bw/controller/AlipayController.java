...
import com.bw.service.impl.thread.ReconverThread;
...
public class AlipayController {
     ...
     * @throws Exception
     */
    static ExecutorService ex = Executors.newFixedThreadPool(8);
    ...
    static ExecutorService recoveryThreadPoll = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());

    @RequestMapping(value = "/alipayNotifyNotice")
    @ResponseBody
    public String alipayNotifyNotice(HttpServletRequest request, HttpServletRequest response) throws Exception {
        String subCode =request.getParameter("sub_code");
        String orderTradeNo = request.getParameter("out_trade_no");
        if(subCode.equals("ACQ.SYSTEM_ERROR")){
            ReconverThread r = new ReconverThread(orderTradeNo,this.orderService);
            recoveryThreadPoll.execute(r);
            return "fail";
       }
...
//			valueStr = new String(valueStr.getBytes("ISO-8859-1"), "utf-8");
		...
		*/
        if (signVerified) {//验证成功
            //商户订单号
            String out_trade_no = new String(request.getParameter("out_trade_no").getBytes("ISO-8859-1"), "UTF-8");
            String sub_code = new String(request.getParameter("sub_code").getBytes("ISO-8859-1"), "UTF-8");
            //支付宝交易号
            String trade_no = new String(request.getParameter("trade_no").getBytes("ISO-8859-1"), "UTF-8");
